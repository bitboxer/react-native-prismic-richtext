...
function serializeImage(element, key, wrapperStyle, style) {
    return createElement(Text, propsWithUniqueKey({}, key), <ImageView element={element} wrapperStyle={wrapperStyle} style={style} accessibilityLabel={element.alt}/>);
}
export const serializerWithStyle = (styles, onLinkPress) => (type, element, text, children, index) => {
    switch (type) {
        ...
        case Elements.image:
            return serializeImage(element, index, styles.imageWrapper, styles.image);
